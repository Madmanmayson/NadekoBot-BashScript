...
if [ $choice -eq 1 ] ; then
	echo ""
	wget -N https://github.com/Madmanmayson/NadekoBot-BashScript/raw/1.9/NadekoARN.sh && bash "$root/NadekoARN.sh"
else
	if [ $choice -eq 2 ] ; then
		echo ""
		wget -N https://github.com/Madmanmayson/NadekoBot-BashScript/raw/1.9/NadekoARU_Latest.sh && bash "$root/NadekoARU_Latest.sh"
	else
			if [ $choice -eq 3 ] ; then
				echo ""
				echo "Exiting..."
